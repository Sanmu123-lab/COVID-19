...
for item in data1:
    ...
    if 'statisticsData' in item.keys():
        ...
        filename = directory + 'countryHistory/' + item['countryFullName'] + UpdateTime + '_history.csv'
...
for dictt in data3:
    ...
    filename = directory + 'chinaProvinceHistory/' + dictt['provinceName'] + UpdateTime + '_history.csv'
    df.to_csv(filename, mode="w", encoding="utf_8_sig", index=False)
